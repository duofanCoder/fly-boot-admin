...
import {saveFlyDictData, updateFlyDictData} from "@/api/system/flyDictData";
...
const handleSave = async (loading: (flag: boolean) => void) => {
  await unref(baseFormRef).instance.validate(async (valid: boolean) => {
    ...
    loading(true);
    try {
      state.isEdit ? await updateFlyDictData(state.formModel) : await saveFlyDictData(state.formModel);
